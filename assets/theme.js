  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _updateAddToCart: function(evt) {
      ...
      if (variant) {
        ...
        var rangeListData = this.productSingleObject.variants,
            container = this.$container,
            var_options = variant.options;

        if(var_options.length < 3) {
          container.find('.swatch-element').removeClass('soldout');
          $.each(variant.options, function (index, value) {
            var opt_num = index + 1,
                opt = 'option'+opt_num,
                data = $.grep(rangeListData, function (e) {
                  return e[opt] === value
                }),
                soldout = $.grep(data, function (e) {
                  return e.available === false
                });

            $.each(soldout, function (i, v) {
              var opt_compare = v.options.slice();
              opt_compare.splice($.inArray(value, opt_compare),1);
              // var is_needed = opt_compare.every(function(val) { return v.options.indexOf(val) >= 0; });
              $.each(opt_compare, function (i, v) {
                container.find('.swatch[data-option-index!="'+ index +'"] .swatch-element.size[data-value="'+ v +'"]').addClass('soldout');
              });
            });
          });
        }
